...
const countNumberOfElements = (input) => input.reduce( (ans, x) => ans ? x++ : x, 0);
...
const countNumberOfChildren = (input) => input.reduce( (ans, x) => x.children ? (ans + x.children.length) : ans, 0);
...
const extractStat = (statName, input) => input.reduce( (ans, x) => x === statName ? ans = x.stat.name : ans);
...
const calculateAverage = (input) => (input.reduce( (ans, x) => ans + x, 0)) / input.length;
...
const extractChildren = input => {
  // const answer = [];
  // let char;
  // char = input.filter( value => /a/.test(value.name));
  // answer.push(char.reduce( (ans, x) => x.children ? ans = x.children : ans));
  // return answer;
};
