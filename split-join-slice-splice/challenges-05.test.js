...
// CHALLENGE 2
//
// Write a function named wordsToCharList that, given a string as input, returns a new array where every element is a
...
const wordsToCharList = (input) => {
  ...
  for(let i = 0; i < input.length; i++) {
    charList.push(input.slice(i, i + 1));
  }
  return charList;
};
...
const listFoods = (recipe) => {
  ...
  for(let i = 0; i < recipe.ingredients.length; i++) {
    word[0] = recipe.ingredients[i].match(/[0-9]+\s[a-z\-]+\s/)[0];
    let num = recipe.ingredients.indexOf(word[0].length - 1);
    result.push(recipe.ingredients[i].slice(num + 1));
  }
  return result;
};
...
const stepActions = (recipe) => {
  ...
  for(let i = 0; i < recipe.steps.length; i++) {
    result.push(recipe.steps[i].match(/[A-Za-z\-]+/)[0]);
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if(numberOfCharacters < 1) {
    return str;
  } else if(numberOfCharacters > str.length - 1) {
    return '';
  } else {
...
const removeVowels = (input) => {
  ...
  for(let i = 0; i < input.length; i++) {
    if(/[aeiou]/.test(input[i])) {
      noVowel = noVowel + input.slice(i, i);
    } else {
