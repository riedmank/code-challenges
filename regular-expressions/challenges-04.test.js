...
const citiesAtoJ = (cities) => {
  ...
  cities.forEach( value => {
    if(value.match(/^[A-J][a-z]*/)) {
      atoJ.push(value.match(/^[A-J][a-z]*/)[0]);
    }
  });
  return atoJ;
};
...
const noPunctuation = (input) => {
  const noPunct = [];
  if(input.match(/[a-z]+\s/ig)) {
    noPunct.push(input.match(/[a-z]+\s/ig));
  }
  return noPunct[0];
};
...
const findShells = (phrase) => {
  const ells = [];
  if(phrase.match(/[a-z]+ells/ig)) {
    ells.push(phrase.match(/[a-z]+ells/ig));
  }
  return ells[0];
};
