...
const getHouses = (arr) => {
  const houses = [];
  const data = getFrom(arr, 'values');
  for(let i = 0; i < data.length; i++) {
    houses.push(data[i].house);
  }
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  let kids = false;
  for(let i = 0; i < data.length; i++) {
    if(data[i].children.length !== 0 && data[i].name === character) {
      kids = true;
    }}
  return kids;
};
...
const hasChildrenEntries = (arr, character) => {
  const data = getFrom(arr, 'entries');
  let kids = false;
  for(let i = 0; i < data.length; i++) {
    if(data[i][1].children.length !== 0 && data[i][1].name === character) {
      kids = true;
    }}
  return kids;
};
...
const houseSize = (arr) => {
  ...
  for(let i = 0; i < data.length; i++) {
    let houseData = {};
    houseData.house = data[i].house;
    let members = 1;
    if(data[i].spouse) {
      members++;
    }
    members += (data[i].children.length);
    houseData.members = members;
...
const houseSurvivors = (arr) => {
  ...
  for(let i = 0; i < data.length; i++) {
    ...
    houseData.house = data[i].house;
    let members = 1;
    if(data[i].spouse && !deceasedSpouses.includes(data[i].spouse)) {
      members++;
    }
    members += (data[i].children.length);
    houseData.members = members;
