...
const validatePin = (pin) => /^\d\d\d\d$/.test(pin);
...
const findTagNames = elements => elements.join().match(/\/\w?\w+/g);
...
const validatePhoneNumber = (phoneNumber) => /^\(?\d\d\d\)?\s?-?\d\d\d-?\s?\d\d\d\d$/.test(phoneNumber);
