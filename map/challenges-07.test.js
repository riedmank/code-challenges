...
const mapTwoToThe = (input) => input.map( value => Math.pow(2, value));
...
const charCode = (input) => input.map( value => value.charCodeAt());
...
const evenOdd = (input) => input.map( value => isNaN(value) ? 'N/A' : (value % 2 === 0 ? 'even' : 'odd'));
...
const extractAbilities = (abilities) => abilities.map( value => value.ability.name);
...
const extractStats = (stats) => stats.map( value => ({name: value.stat.name, total: value.effort + value.baseStat,}));
