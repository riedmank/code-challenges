...
const count = (target, input) => (input.map( value => value.reduce( (ans, x) => x === target ? ans + 1 : ans, 0))).reduce( (ans, x) => ans + x, 0);
...
const totalSum = (input) => (input.map( value => value.reduce( (ans, x) => ans + x, 0))).reduce( (ans, x) => ans + x, 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map( value => value.filter( value => value % 5 === 0 && typeof(value) === 'number').map( value => Math.pow(2, value)));
...
let findMaleAndFemale = (data) => (data.filter( value => value.gender === 'female' || value.gender === 'male')).reduce( (ans, x) => ans.concat(x.name), []).join(' and ');
