...
const oddValues = (input) => input.filter( value => value % 2 !== 0);
...
const filterStringsWithVowels = (input) => input.filter( value => /[aeiou]/.test(value));
...
const notInFirstArray = (forbiddenValues, input) => input.filter( (value) => !forbiddenValues.includes(value));
...
const getBaseStatGreaterThan = (input, minBaseStat) => input.filter( value => value.baseStat > minBaseStat);
...
const getStatName = (input, minBaseStat) => input.filter( value => value.baseStat > minBaseStat).map( value => value.stat.name);
...
const getCharactersWithoutChildren = (input) => input.filter( value => !value.hasOwnProperty('children'));
...
const evenOddNumericValues = (input) => input.filter( value => typeof value === 'number').map( value => (value % 2 === 0) ? 'even' : 'odd');
