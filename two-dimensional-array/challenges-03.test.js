...
const averageDailyTemperature = (weather) => {
  ...
  for(let i = 0; i < weather.length; i++) {
    for(let j = 0; j < weather[i].length; j++) {
      monthAvg += weather[i][j];
      numDays++;
    }
  }
  return (monthAvg / numDays);
};
...
let lowestWeeklyTemperatureData = [
  [33, 64, 58, 65, 71, 57, 60], //58.28
  [40, 45, 33, 53, 44, 59, 48], //46
  [55, 54, 60, 53, 59, 57, 61], //57
  [65, 56, 55, 52, 55, 62, 57]  //57.42
];
...
// Writing that helpCheck function to check every possible win line is way easier than writing for loops!
//
// Here is a sample board:
// [
//   ['X', '', 'O'],
...
const minesweeper = (board) => {
  const helpArray = new Array(board.length);
  helpArray.forEach( () => helpArray.push(new Array(board[0].length)));
};
