...
const firstLetters = (strs) => strs.map( val => val.substring(0, 1));
...
const findHappiness = (strs) => strs.filter(val => val.includes(':)'));
...
const standardizePhoneNumbers = (phoneNumbers) => phoneNumbers.map( val => val.substring(val.indexOf('(') + 1, val.indexOf(')')) + val.substring(val.indexOf(')') + 2, val.indexOf('-')) + val.substring(val.indexOf('-') + 1));
...
const allHappy = (strs) => strs.every( val => val.includes(':)'));
...
const findAnything = (strs, target) => strs.filter( val => val.includes(target));
...
const allAnything = (strs, target) => strs.every( val => val.includes(target));
...
const unenrollBrook = (roster) => roster.map( val => val.filter( name => !name.includes('Brook')));
...
const sortByDay = (eventStrings) => {
  const answer = [];
  for(let i in eventStrings) {
    const subAnswer = [];
    for(let j in eventStrings) {
      if (eventStrings[j].includes(daysOfWeek[i])) {
        subAnswer.push(eventStrings[i]);
      }
    }
...
const characterByIndex = (strs) => strs.map( (val, i) => val.charAt(i));
